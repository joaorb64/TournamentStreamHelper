    ...
    if (!oldData.score ||
      JSON.stringify(data.score.history_sets) != JSON.stringify(oldData.score.history_sets)) {
        ...
        Object.values(data.score.history_sets["1"]).slice(0, 6).forEach((sets, s) => {
          ...
          Object.values(data.score.history_sets["1"]).slice(0, 6).forEach((tournament, s) => {
            SetInnerHtml($(`.player1_content .tournament${s+1} .info .tournament_info .tournament_name`), tournament.tournament_name);
            SetInnerHtml($(`.player1_content .tournament${s+1} .info .tournament_info .event_name`), tournament.event_name);
            SetInnerHtml($(`.player1_content .tournament${s+1} .info .tournament_logo`), 
            `
              <span class="logo" style="background-image: url('${tournament.tournament_picture}')"></span>
            `
            );
            SetInnerHtml($(`.player1_content .tournament${s+1} .info .placement`), getNumberWithOrdinal(tournament.placement));
