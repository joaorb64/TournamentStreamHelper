(($) => {
    let startingAnimation = gsap.timeline({ paused: true })
        .from(['.container'], { duration: 1, width: '0', padding: "0", ease: "power2.inOut" }, 0)
    ...
    async function Update(){
        ...
        [data.score.team["1"], data.score.team["2"]].forEach((team, t) => {
            [team.players["1"]].forEach((player, p) => {
                if(player){
                    SetInnerHtml($(`.p${t+1}.container .name`), `
                        <span>
                            <span class='sponsor'>
                    ...
                    SetInnerHtml($(`.p${t+1}.container .flagstate`),
                        player.state.asset ? `<div class='flag' style='background-image: url(../../${player.state.asset})'></div>` : ""
                    );
                    ...
                    SetInnerHtml($(`.p${t+1}.container .avatar`),
                        player.avatar ? `<div style='background-image: url(${player.avatar})'></div>` : ""
                    );
            
                    SetInnerHtml($(`.p${t+1}.container .twitter`), String(player.twitter));
