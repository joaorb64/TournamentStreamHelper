(($) => { 
    ...
    async function Update(){
        ...
        if(Object.keys(oldData).length == 0 || Object.keys(oldData.commentary).length != Object.keys(data.commentary).length){
            let html = "";
        ...
        Object.values(data.commentary).forEach((commentator, index)=>{
            SetInnerHtml($(`.commentator${index} .name`), `
                <div class="mic_icon"></div>
                <span class="team">${commentator.team ? commentator.team + "&nbsp;" : ""}</span>${commentator.name}
            `);
            SetInnerHtml($(`.commentator${index} .real_name`), commentator.real_name);
